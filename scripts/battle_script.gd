...
const MarkerPrefab = preload("res://scenes/marker.tscn")
...
func enemy_prepare_turn():
	# TODO: Low tech solution - removes all modifiers, works fine for 1-turn effects,
	# but anything else will need something more complex
	tile_modifiers.clear()
	for active_marker in active_markers:
		active_marker.queue_free()
	active_markers.clear()
	var msg = "The enemy prepares its Skill " + enemy.chosen.name + "!"
	log_container.add_log_event(msg)
