...
func _process(delta):
	...
	if horizontal:
		...
		if (
			(current_speed > 0 and position.x > screen_size.x + sprite_dim)
			or (current_speed < 0 and position.x < -sprite_dim)
		):
		...
		if (
			(current_speed > 0 and position.y > screen_size.y + sprite_dim)
			or (current_speed < 0 and position.y < -sprite_dim)
		):
			_reset_character(false)
