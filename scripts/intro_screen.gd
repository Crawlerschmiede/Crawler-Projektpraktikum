...
func play_intro_sequence():
	...
	tween.tween_property(static_rect, "scale:y", 1.0, 1.0).set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_OUT)
...
func apply_impact_shake(duration: float, force: float):
	...
	for i in range(12):
		var offset = Vector2(randf_range(-force, force), randf_range(-force, force))
